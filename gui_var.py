...
# --- MATEMÁTICA Y LÓGICA DE DIBUJO ---
def calcular_homografia_y_fuga(pts, modo):
    pts_src = np.array(pts, dtype="float32")
    if modo == "GRANDE":
        pts_dst = np.array([[0, 4032], [0, 0], [1650, 0], [1650, 4032]], dtype="float32")
    else:
        pts_dst = np.array([[0, 1832], [0, 0], [550, 0], [550, 1832]], dtype="float32")
        
    H, _ = cv2.findHomography(pts_src, pts_dst)
    punto_fuga = None
    if H is not None:
        H_inv = np.linalg.inv(H)
        vp = np.dot(H_inv, [0, 1, 0])
        if vp[2] != 0:
            punto_fuga = (int(vp[0]/vp[2]), int(vp[1]/vp[2]))
    return H, punto_fuga

def calcular_distancia_cm(pt_def, pt_att, H):
...
class VarProApp(ctk.CTk):
    def __init__(self):
        super().__init__()

        self.title("VAR Pro - LaLiga Edition")
    ...
    def toggle_area(self):
        self.modo_calibracion = "GRANDE" if self.switch_area.get() else "PEQUEÑA"
        self.actualizar_dibujos()
    ...
    def actualizar_dibujos(self):
        ...
        fases_text = ["Fase 1: Calibrar Área", "Fase 2: Defensores", "Fase 3: Atacantes", "Fase 4: Límites del Campo"]
        self.lbl_fase.configure(text=fases_text[self.fase])
        
        if len(self.pts_fuga) >= 4:
            self.matriz_homografia, self.punto_fuga = calcular_homografia_y_fuga(self.pts_fuga[0:4], self.modo_calibracion)

        if self.mostrar_lineas_fuga and len(self.pts_fuga) >= 4:
            for i in range(4):
                cv2.line(self.imagen_base_dibujada, self.pts_fuga[i], self.pts_fuga[(i+1)%4], LALIGA_BLANCO, 1, cv2.LINE_AA)
        ...
        if self.punto_fuga is not None and mejor_def_proyectado is not None and self.matriz_homografia is not None and len(self.pts_att) >= 2:
            y_eval = self.pts_def[1][1]
            for i in range(0, len(self.pts_att) - 1, 2):
                hombro, pie = self.pts_att[i], self.pts_att[i+1]
                p_proyectado, _ = obtener_punto_mas_adelantado(hombro, pie, self.punto_fuga, y_eval, self.ataca_derecha)
                dist_cm = calcular_distancia_cm(mejor_def_proyectado, p_proyectado, self.matriz_homografia)
                color = LALIGA_VERDE if dist_cm <= 0 else LALIGA_ROJO
                ...
                cv2.putText(self.imagen_base_dibujada, texto_cm, (hombro[0] + 10, hombro[1] - 10), cv2.FONT_HERSHEY_DUPLEX, 0.6, LALIGA_NEGRO, 2, cv2.LINE_AA)
                cv2.putText(self.imagen_base_dibujada, texto_cm, (hombro[0] + 10, hombro[1] - 10), cv2.FONT_HERSHEY_DUPLEX, 0.6, color, 1, cv2.LINE_AA)
                if dist_cm > 0:
